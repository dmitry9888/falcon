    ...
    for (;;) {
        ...
        if (req.request_type() == tzr_proto::TxRequest::TXINPUT) {
            if (req.details().has_tx_hash()) {
                ...
                if (s.length() < 32) {
                    return errorN(1, sError, __func__, "TXINPUT tx_hash is too short %d.", s.length());
                }
                std::reverse(s.begin(), s.end());
                uint256 req_txhash;
                memcpy(req_txhash.begin(), s.data(), 32);
                ...
                if (cti == m_tx_cache.end()) {
                    return errorN(1, sError, __func__, "TXINPUT: Unknown prevout txn requested %s.", req_txhash.ToString());
                }
                const CMutableTransaction &prev_tx = cti->second;
                if (n >= prev_tx.vin.size()) {
                    return errorN(1, sError, __func__, "TXINPUT req_tx has too few inputs %d.", n);
                }
            ...
            if (req.details().has_tx_hash()) {
                ...
                std::string s = req.details().tx_hash();
                if (s.length() < 32) {
                    return errorN(1, sError, __func__, "TXOUTPUT tx_hash is too short %d.", s.length());
                }
                std::reverse(s.begin(), s.end());
                COutPoint prevout;
                memcpy(prevout.hash.begin(), s.data(), 32);
                prevout.n = req.details().request_index();
                const auto &cti = m_tx_cache.find(prevout.hash);
                if (cti == m_tx_cache.end()) {
                    return errorN(1, sError, __func__, "TXOUTPUT: Unknown prevout txn requested %s.", prevout.hash.ToString());
                }
                const CMutableTransaction &prev_tx = cti->second;

                if (prevout.n >= prev_tx.vpout.size()) {
                    return errorN(1, sError, __func__, "TXOUTPUT: Unknown prevout requested %s.", prevout.ToString());
                }
                const auto msg_tx = msg.mutable_tx();
                ...
                if (send_output->IsType(OUTPUT_STANDARD)) {
                    ...
                    // TODO: Send fake data?
                    return errorN(1, sError, __func__, "Unknown output type.");
                }
            } else {
                ...
                if (send_output->IsType(OUTPUT_STANDARD)) {
                    ...
                    if ((int)i == change_pos) {
                        ...
                        std::string addr;
                        int rv = SetAddressFromScript(addr, sError, pscript);
                        if (0 != rv) {
                            return rv; // sError is set
                        }
            ...
            std::string s = req.details().tx_hash();
            if (s.length() < 32) {
                return errorN(1, sError, __func__, "TXMETA tx_hash is too short %d.", s.length());
            }
            std::reverse(s.begin(), s.end());
            uint256 req_txhash;
            memcpy(req_txhash.begin(), s.data(), 32);
            ...
            const auto &cti = m_tx_cache.find(req_txhash);
            if (cti == m_tx_cache.end()) {
                return errorN(1, sError, __func__, "TXMETA: Unknown prevout txn requested %s.", req_txhash.ToString());
            }
            const CMutableTransaction &prev_tx = cti->second;
